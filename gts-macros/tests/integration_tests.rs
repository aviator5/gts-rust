...
fn test_gts_id_segments_match_schema() {
    ...
    // Parse it with GtsId
    let gts_id = GtsId::try_new(schema_id_str).expect("Schema ID should be valid");
...
fn test_gts_id_segments_match_instance() {
    ...
    // Parse it with GtsId
    let gts_id = GtsId::try_new(&instance_id_str).expect("Instance ID should be valid");
...
fn test_schema_and_instance_segments_relationship() {
    ...
    let schema_id = GtsId::try_new(EventTopicV1::gts_type_id().as_ref()).unwrap();

    // An instance ID from the macro
    let instance_id_str = EventTopicV1::gts_make_instance_id("x.core.idp.contacts.v1");
    let instance_id = GtsId::try_new(&instance_id_str).unwrap();
...
fn test_entity_and_gts_id_vendor_package_namespace_match() {
    ...
    let direct_gts_id = GtsId::try_new(EventTopicV1::gts_type_id().as_ref()).unwrap();
