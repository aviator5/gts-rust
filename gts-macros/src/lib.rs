...
/// Extract the parent schema ID from a `type_id` (removes the last segment)
...
/// Check if a type is `GtsTypeId` (either directly or as a path)
fn is_type_gts_type_id(ty: &syn::Type) -> bool {
    is_type_named(ty, "GtsTypeId")
}
...
fn validate_field_types(
    ...
    let has_valid_type_field = fields.iter().any(|field| {
        ...
        is_type_field && is_type_gts_type_id(&field.ty)
    });
...
fn validate_version_match(struct_ident: &syn::Ident, type_id: &str) -> syn::Result<()> {
    ...
    let schema_version = extract_type_id_version(type_id);
        ...
        while !input.is_empty() {
            ...
            match key.to_string().as_str() {
                ...
                "type_id" => {
                    let value: LitStr = input.parse()?;
                    let id = value.value();
                    // Type-id-specific check: must end with ~
                    if !id.ends_with('~') {
                        return Err(syn::Error::new_spanned(
                            value,
                            format!(
                                "struct_to_gts_schema: Invalid GTS Type Identifier: must end with '~' (type marker), got '{id}'"
                            ),
                        ));
                    }
                    ...
                    if let Err(e) = gts_id::validate_gts_id(&id, false) {
                        let msg = match &e {
                            gts_id::GtsIdError::Id { cause, .. } => {
                                format!("Invalid GTS Type Identifier: {cause}")
                            }
                ...
                _ => {
                    return Err(syn::Error::new_spanned(
                        key,
                        "Unknown attribute. Expected: dir_path, type_id, description, properties, or base",
                    ));
                }
            }
...
///
/// - `gts_type_id() -> &'static GtsTypeId` - Get the struct's GTS Type Identifier
/// - `gts_base_type_id() -> Option<&'static GtsTypeId>` - Get parent type identifier (None for base structs)
...
pub fn struct_to_gts_schema(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // Validate version match between struct name suffix and type_id
    ...
    // Validate base attribute consistency with type_id segments
    ...
    // Build the schema output file path from dir_path + type_id
    ...
    let base_type_id_const = if let Some(parent_id) = &expected_parent_type_id {
        quote! {
            /// Parent schema ID (extracted from type_id segments). Use `gts_base_type_id()` instead.
            #[doc(hidden)]
            #[allow(dead_code)]
        ...
        quote! {
            /// Parent schema ID (None for base types). Use `gts_base_type_id()` instead.
            #[doc(hidden)]
            #[allow(dead_code)]
    ...
    // Generate the literal option value for use in static initializers (avoids Self::BASE_TYPE_ID)
    let base_type_id_option = if let Some(parent_id) = &expected_parent_type_id {
    ...
    let base_assertion = match &args.base {
        BaseAttr::Parent(parent_ident) => {
            ...
            let type_id_assertion_msg = format!(
                "struct_to_gts_schema: Base struct '{parent_ident}' schema ID must match parent segment '{parent_id}' from type_id"
            );
            ...
            quote! {
                // Compile-time assertion: verify parent struct's GTS_TYPE_ID matches expected parent segment
                ...
                if let Some(props_obj) = properties.as_object_mut() {
                    for (_key, value) in props_obj.iter_mut() {
                        if let Some(ref_str) = value.get("$ref").and_then(|v| v.as_str()) {
                            ...
                            } else if ref_str == "#/$defs/GtsTypeId" {
    ...
    let expanded = quote! {
        ...
        impl #impl_generics #struct_name #ty_generics #gts_schema_where_clause {
            ...
            /// Get the parent (base) GTS Type Identifier as a static reference.
            ...
            pub fn gts_base_type_id() -> Option<&'static ::gts::gts::GtsTypeId> {
                static BASE_TYPE_ID: std::sync::LazyLock<Option<::gts::gts::GtsTypeId>> =
                    std::sync::LazyLock::new(|| {
                        #base_type_id_option.map(::gts::gts::GtsTypeId::new)
                    });
                BASE_TYPE_ID.as_ref()
            }
